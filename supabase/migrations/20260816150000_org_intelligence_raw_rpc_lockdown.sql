-- Gate 3.4 P0 — Lock down raw organisation intelligence aggregation RPC.
-- EXECUTE grants only (plus minimal service_role auth gate so internal callers
-- work: service_role JWTs have auth.uid() = null).
-- Does NOT change aggregation SELECT logic, tables, or data.
-- Lead/authenticated must not call this RPC; server uses service_role after authz.
...
begin
  -- Gate 3.4 P0: raw aggregation is internal-only (service_role EXECUTE).
  -- Application must authorise intelligence.organisation.read before calling.
  -- Aggregation SELECT body is unchanged.
...
comment on function public.aggregate_organisation_intelligence_sources(uuid, date, date) is
  'INTERNAL: anonymised organisation aggregates for snapshot generation. Callable only via service_role after application authz (intelligence.organisation.read). Includes authorised living development_evidence capability keys (opaque contributor keys). Excludes self-development. Never returns private identity, session notes, evidence text or confidential references. Must not be exposed to Lead/browser.';
